...
void initFirebase(SystemStatus &status)
{
    ...
    if (Firebase.RTDB.setString(&fbData, "/test/connection", "esp32_test"))
    {
        ...
        if (Firebase.RTDB.getString(&fbData, "/test/connection"))
        {
            ...
            // Initialize default control values if they don't exist
            Serial.println("Setting up default control values...");
            if (!Firebase.RTDB.getFloat(&fbData, "/control/target_temperature"))
            {
                Serial.println("Setting default target temperature: 22.0°C");
                Firebase.RTDB.setFloat(&fbData, "/control/target_temperature", 22.0);
            }

            if (!Firebase.RTDB.getBool(&fbData, "/control/heater_enabled"))
            {
                Serial.println("Setting default heater enabled: true");
                Firebase.RTDB.setBool(&fbData, "/control/heater_enabled", true);
            }

            // Set some additional useful control values
            Firebase.RTDB.setFloat(&fbData, "/control/temperature_tolerance", 1.0); // ±1°C tolerance
            Firebase.RTDB.setInt(&fbData, "/control/update_interval", 15);          // 15 second updates
            Firebase.RTDB.setString(&fbData, "/control/mode", "auto");              // auto/manual mode

            Serial.println("Default control values initialized");
        }
        else
        {
...
void handleFirebase(SystemStatus &status)
{
    if (!fbInitialized)
    {
        if (WiFi.status() == WL_CONNECTED)
        {
            // Rate limit initialization attempts to avoid spam
    ...
    // Rate limit Firebase status checks to avoid spam
...
void fetchControlValuesFromFirebase()
{
    ...
    Serial.println("Fetching control values from Firebase...");

    // Read temperature setpoint
    if (Firebase.RTDB.getFloat(&fbData, "/control/target_temperature"))
    {
        float targetTemp = fbData.floatData();
    ...
    {
        Serial.println("Failed to read target temperature");
    }

    // Read heater enable/disable
    if (Firebase.RTDB.getBool(&fbData, "/control/heater_enabled"))
    {
        bool heaterEnabled = fbData.boolData();
        Serial.print("Heater enabled: ");
        Serial.println(heaterEnabled ? "true" : "false");
    }
    else
    {
        Serial.println("Failed to read heater enabled status");
    }

    // Read temperature tolerance
    if (Firebase.RTDB.getFloat(&fbData, "/control/temperature_tolerance"))
    {
        float tolerance = fbData.floatData();
        Serial.print("Temperature tolerance: ±");
        Serial.print(tolerance);
        Serial.println("°C");
    }

    // Read update interval
    if (Firebase.RTDB.getInt(&fbData, "/control/update_interval"))
    {
        int interval = fbData.intData();
    ...
    // Read control mode
    if (Firebase.RTDB.getString(&fbData, "/control/mode"))
    {
        String mode = fbData.stringData();
        Serial.print("Control mode: ");
        Serial.println(mode);
    }

    // Read current sensor data back for verification
